...
A proposta do trabalho é implemetar um programa usando a biblioteca pthread na linguam ```C```, que consegua atender as exigências acima, além de garantir a inanição (Starvation) de threads. O trabalho é composto por duas partes. Sendo a primeira o programa principal, uma solução para o problema de escritores/leitores com garantia de ausencia de inanição das threads. E a segunda, um programa auxiliar para verificar, através de uma leitura do log de execução gerado pelo programa principal, se a execução ocorreu com sucesso.
